...
// const concurrent=1;
// const step=10000;
// const concurrentInsert = (process,advance) => {
//   var hundred=process+advance*concurrent;
//   var seedData = sourceData(hundred);
//   Grubhub.insertMany(seedData).then(()=> {
//     if(hundred%1000===0)showTime(hundred+1+"00");
//     if(hundred===concurrent*step-1)showTime("done("+(hundred+1)+"00)");
//     if(advance<step-1)concurrentInsert(process,advance+1);
//   })
//   .catch((err)=> console.log(err))
// }
// for(var i=0;i<concurrent;i++)
//   concurrentInsert(i,0)

// const size=100;
// const group=10;
// const promiseInsertion = (advance)=>{
//   var seedData=[];
//   for(var i=0;i<size;i++)
//     seedData.push(...sourceData(i+advance*size));
//   Grubhub.insertMany(seedData).then(()=>{
//     showTime("group "+(advance+1)+" done");
//     if(advance<group-1)promiseInsertion(advance+1);
//   });
// };
// promiseInsertion(0);
...
// var importSeeder = (step)=>{
//   var str=str100Data(step);
//   fs.appendFile('db/mongo/importer.json',str,(err)=>{
//     if(err)console.log("!");
//     else{
//       if(step%1000===0)showTime("step "+(step+1)+" done");
//       if(step<group-1)importSeeder(step+1);
//       else showTime("done");
//     }
//   });
// }
...
var stream=fs.createWriteStream('db/mongo/importTest.json');
var i=0;
var runStream=()=>{
  stream.write(str100Data(i));
  i++;
  if(i<group)stream.once('drain',runStream);
